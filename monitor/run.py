...
from monitor.browser_utils import new_page, close_browser
...
def run_once(max_matches: int = 12):
    ...
    try:
        _run_once_inner(
            max_matches, previous_matches, new_matches_state, all_alerts, dashboard_rows
        )
    finally:
        # Chiude il browser/driver Playwright condiviso (vedi browser_utils.py):
        # va fatto una volta a fine esecuzione, sia in caso di successo che di
        # errore, altrimenti il processo puo' restare "appeso" in GitHub Actions.
        close_browser()
...
def _run_once_inner(max_matches, previous_matches, new_matches_state, all_alerts, dashboard_rows):
